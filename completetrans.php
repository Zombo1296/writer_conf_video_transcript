<?php
	//Bohan
	//save trascriber's transcription
...
$id1=$_GET["id1"];
$id2=$_GET["id2"];	
$query = "select userID1,status from video2 where id1 = $id1 and id2 = $id2";
...
if($_POST['finished']=='yes'){	
	...
	$id1=$_GET["id1"];
	$id2=$_GET["id2"];
	$query = "update video2 set text = '$text' where id1 = $id1 and id2 = $id2";
	$result = query_writers_db($query);
	$query = "update video2 set status = 3 where id1 = $id1 and id2 = $id2";
	$result = query_writers_db($query);
	echo "<a href='video.php?id1=$id1&id2=$id2'>success,click here to continue</a>";
//if not finished, just save transcription,don't change status
